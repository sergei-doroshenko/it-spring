	...
	public void execute() throws IOException, ServletException {
		...
		try {
			...
			if (commandName.equals(Constants.COMMAND_SUBMIT_ISSUE)) {
				session.removeAttribute(Constants.ISSUE);
			} else if (commandName.equals(Constants.COMMAND_EDIT_ISSUE)) {

				if (session.getAttribute(Constants.ISSUE) == null) {
					jump(Constants.URL_MAIN);
					return;
				}
